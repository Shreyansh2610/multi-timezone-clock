...
export function ensureTimezoneCache() {
    ...
    try {
            ...
            const traverse = (loc) => {
                ...
                if (loc.has_timezone()) {
                    ...
                    if (tzStr) {
                        ...
                        if (!timezoneCache.has(tzStr)) {
                            timezoneCache.set(tzStr, {
                                countryCode: countryCode || null,
                                cityName: cityName || tzStr,
                            });
                        }
                    }
                }
        ...
        console.error('Failed to populate GWeather timezone cache:', e);
    }
}
...
export function getFlag(zone) {
    if (FLAGS[zone])
        return FLAGS[zone];
    ensureTimezoneCache();
    const cached = timezoneCache.get(zone);
    if (cached && cached.countryCode)
        return getFlagEmoji(cached.countryCode);
    return '🌍';
}
