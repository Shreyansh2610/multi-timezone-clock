...
const TimezoneRow = GObject.registerClass({
    GTypeName: 'MultiClockTimezoneRow',
...
const TimezonesPage = GObject.registerClass({
    GTypeName: 'MultiClockTimezonesPage',
    ...
    constructor(settings) {
        ...
        try {
            ...
            if (world) {
                ...
                const traverse = (loc) => {
                    ...
                    if (loc.has_timezone()) {
                        const tzStr = loc.get_timezone_str();
                        if (tzStr && this._isValidTimezone(tzStr) && !added.has(tzStr)) {
                            added.add(tzStr);
                            const countryCode = loc.get_country() || '';
                            const countryName = loc.get_country_name() || '';
                            const cityName = loc.get_name() || '';

                            const searchText = `${tzStr} ${countryCode} ${countryName} ${cityName}`.toLowerCase();
                            const iter = listStore.append();
                            listStore.set(iter, [0, 1], [tzStr, searchText]);
                        }
                    }

                    let child = null;
                    while ((child = loc.next_child(child)) !== null)
                        traverse(child);
                };
                traverse(world);
                populated = added.size > 0;
            }
        } catch (e) {
            console.error('Failed to load system timezones via GWeather:', e);
        }

        if (!populated) {
            try {
                const timezones = Intl.supportedValuesOf('timeZone')
                    .filter(tz => this._isValidTimezone(tz));
                for (const tz of timezones) {
                    const iter = listStore.append();
                    listStore.set(iter, [0, 1], [tz, tz.toLowerCase()]);
                }
            } catch (e) {
                console.error('Failed to load system timezones via Intl:', e);
                const fallbackZones = [
                    'UTC',
                    'Asia/Kolkata',
                    'America/New_York',
        ...
        completion.set_match_func((comp, key, iter) => {
            const model = comp.get_model();
            const val = model.get_value(iter, 1);
            if (!val || !key)
                return false;
            return val.includes(key.toLowerCase());
        });
        this._entry.set_completion(completion);
...
function createGeneralPage(settings) {
    ...
    addSwitchRow(displayGroup, settings, 'show-date', 'Show Date');
    addSwitchRow(displayGroup, settings, 'show-weekday', 'Show Weekday');
    addSwitchRow(displayGroup, settings, 'show-seconds', 'Show Seconds');
    addSwitchRow(displayGroup, settings, 'show-flags', 'Show Flags');
    addSwitchRow(displayGroup, settings, 'show-timezone-name', 'Show Timezone Name');
    ...
    page.add(popupGroup);
    addSwitchRow(popupGroup, settings, 'popup-date', 'Popup Date');
    addSwitchRow(popupGroup, settings, 'popup-seconds', 'Popup Seconds');

    return page;
}

function createPanelPage(settings) {
    const page = new Adw.PreferencesPage({
        title: 'Panel',
    ...
    addSwitchRow(layoutGroup, settings, 'compact-mode', 'Compact Mode');

    return page;
}

function createAboutPage(settings) {
    const page = new Adw.PreferencesPage({
        title: 'About',
    ...
    infoGroup.add(new Adw.ActionRow({
        title: 'GitHub',
        subtitle: 'https://github.com/Shreyansh2610/multi-timezone-clock',
    }));
    infoGroup.add(new Adw.ActionRow({
        title: 'License',
...
export default class MultiTimezoneClockPreferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        window.add(createPanelPage(settings));
        window.add(timezonesPage);
        window.add(createAboutPage(settings));
